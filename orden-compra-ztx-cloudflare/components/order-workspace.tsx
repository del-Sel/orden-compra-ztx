...
function automaticFinalStatus(stage: Stage, totalQuantity: number, deliveries: Delivery[]) {
  if (stage !== 'signed' || deliveries.length === 0) return 'Pendiente de entrega';
  const registeredQuantity = deliveries.reduce((sum, delivery) => sum + delivery.quantity, 0);
